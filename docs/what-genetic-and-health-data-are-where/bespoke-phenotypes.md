...
At present, Genes & Heatlh provides a curated set of 285 custom-defined binary phenotypes as detailed in the "Phenotype curation" section (we also provide 'First Occurrence of 3digitICD10' in the style of UK Biobank).  We are aware that TRE users sometimes which to use bespoke phenotypes based on their own curated codelist(s).  We are currently developing this facility but in the interim, users wishing to identify a list of G&H volunteers matchig their codelist(s) can do so using intermediary files generated by the [BI_PY pipeline](https://github.com/genes-and-health/BI_PY/){target="_blank"}.   
